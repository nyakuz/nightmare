...
namespace Nightmare.Main {
  public class MainMiddleware {
    ...
    public class PageSapiContext: PageSapi, IPage {
      ...
      public readonly CancellationToken RequestAborted;
      ...
      public override Task ResponseFile(string filepath, string fileext) {
        ...
        return Ctx.Response.SendFileAsync(filepath, RequestAborted);
      }
      ...
      public async override ValueTask<int> SendFlushCallback() {
        await Ctx.Response.BodyWriter.FlushAsync(RequestAborted);
        return 0;
      }
      public override long ReadPostCallback(Span<byte> buf, long count_bytes) {
        var count_bytes2 = (int)count_bytes;
        int buf_size = 0;
        var req_stream = Ctx.Request.BodyReader.AsStream(true);

        int read_size;
        do {
          var span = buf.Slice(buf_size, count_bytes2 - buf_size);
          read_size = req_stream.Read(span);
          buf_size += read_size;
          if(read_size <= 0) {
            Finish();
            break;
          }
        } while(buf_size < count_bytes);

        return buf_size;

        async void Finish() {
          await req_stream.DisposeAsync();
          await Ctx.Request.BodyReader.CompleteAsync();
        }
      }
    ...
    public async Task InvokeAsync(HttpContext context, MainRouter router, MimeTypeSingleton mimeType, ISecureService session) {
      ...
      if(res.Headers.ContentType.Count == 0 && res.StatusCode == 404 && res.HasStarted == false) {
        res.StatusCode = 200;
